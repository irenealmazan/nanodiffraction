...
filename_tosave = 'results/data_scan_onlyread_nopad';%'results/data_scan_test';
filename_toload = 'results/data_scan_onlyread_nopad';
%filename_toload = 'results/data_scan_test';
...
if flag_doAlignment == 1
    ...
    save([filename_tosave '_alignement_structure' num2str(lst(1)) '_' num2str(lst(end)) '.mat'],'XRF_aligned','-v7.3');
...
if flag_analyze == 1
    ...
    save([filename_tosave '_analysis_aligned' num2str(lst(1)) '_' num2str(lst(end)) '.mat'],'dat','XRF_aligned','centroid_struct','struct_centroidShift','-v7.3');
...
if flag_display == 1
   ...
   [dat.rock_curve,dat.thetalist] = ND_display_data.displayRockCurve(dat,'do_mask',1,'figNum',9000);
    ...
    ND_display_data.display2Dmap(centroid_struct.Xcentroids,'figNum',figNum+22,'Xval',dat.xsuperGrid_s(1,:),'Yval',dat.ysuperGrid_s(:,1),'window',window_for_maps,'figTitle','Column Centroids','font',30,'size_figure',[90 151 1203 510]);caxis([100 135]);
    figure(figNum+22);  hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),mask0(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),1,'-r','LineWidth',1.0);

    ND_display_data.display2Dmap(centroid_struct.Ycentroids.*mask_nan,'figNum',figNum+23,'Xval',dat.xsuperGrid_s(1,:),'Yval',dat.ysuperGrid_s(:,1),'window',window_for_maps,'figTitle','Rows Centroids ','font',30,'size_figure',[90 151 1203 510]);caxis([220 250]);
    ...
    ND_display_data.display2Dmap(centroid_struct.Thcentroids.*mask_nan,'figNum',figNum+24,'Xval',dat.xsuperGrid_s(1,:),'Yval',dat.ysuperGrid_s(:,1),'window',window_for_maps,'figTitle','Theta Centroids ','font',30,'size_figure',[90 151 1203 510]);caxis([84.28 84.5])
    ...
    ND_display_data.display2Dmap(struct_centroidShift.strain.*mask_nan,'figNum',figNum+25,'Xval',dat.xsuperGrid_s(1,:),'Yval',dat.ysuperGrid_s(:,1),'window',window_for_maps,'figTitle','Strain','font',30,'size_figure',[90 151 1203 510]);%caxis([220 250]);
...
initial_index = 1;
     ...
     for ii = 1:numel(dat.scan)
        ...
        if ii > initial_index
            xshifts_ini(ii) = 5;%XRF_aligned.scan(ii).xshifts-10;%(ii-initial_index-1)*5;
            yshifts_ini(ii) = 3;%XRF_aligned.scan(ii).yshifts;
        else            
    ...
    XRF_aligned_2 = ND_data_processing.doAlignment(XRF_struct,'scanid',lst,'thetalist',angs,'initial_index',initial_index,'xshifts_ini',xshifts_ini,'yshifts_ini',yshifts_ini,'do_align',1,'do_Ref_XRF0',1);
...
%{
pixel = [14,41]; % rows and columns, Y and X

figure(101);
clf;
imagesc(dat.ii(pixel(1)).jj(pixel(2)).im);%(ROIxstart:ROIxsize+ROIxstart,ROIystart:ROIysize+ROIystart));
axis image;
colormap hot;
colorbar;
title(['summed ccd']);
...
[ccd_to_plot,sum_ccd] = ND_read_data.readCCD_oneScan_onepixel(datapath,lst9,innerpts,pixel);   

% plot ccds of rock curve for the given pixel
 
figure(100);
clf;
for kk = 9%1:numel(ccd_to_plot)
    imagesc(ccd_to_plot(kk).ccd);%(ROIxstart:ROIxsize+ROIxstart,ROIystart:ROIysize+ROIystart));
    axis image;
    colormap hot;
    colorbar;
    title(['th = ' num2str(dat9.thetalist(kk)) 'deg']);
    %pause(.5);
end
