...
if flag_display == 1
   ...
   [dat.rock_curve,dat.thetalist] = ND_display_data.displayRockCurve(dat,'do_mask',1,'figNum',9000);
    ...
    figNum =4000;
    ND_display_data.display2Dmap(dat.map2D_SumInt,'figNum',figNum,'Xval',dat.xsuperGrid_s(1,:),'Yval',dat.ysuperGrid_s(:,1),'window',window_for_maps,'figTitle','Integrated intensity','font',30,'size_figure',[90 151 1203 510]);%caxis([220 250]);
    ...
    ND_display_data.display2Dmap(centroid_struct.Xcentroids.*mask,'figNum',figNum+22,'Xval',dat.xsuperGrid_s(1,:),'Yval',dat.ysuperGrid_s(:,1),'window',window_for_maps,'figTitle','Row Centroids','font',30,'size_figure',[90 151 1203 510]);caxis([220 250]);
    figure(figNum+22);  hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),mask(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),'-r','LineWidth',1.0);

    ND_display_data.display2Dmap(centroid_struct.Ycentroids.*mask,'figNum',figNum+23,'Xval',dat.xsuperGrid_s(1,:),'Yval',dat.ysuperGrid_s(:,1),'window',window_for_maps,'figTitle','Column Centroids ','font',30,'size_figure',[90 151 1203 510]);caxis([100 135]);
    figure(figNum+23);  hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),mask(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),'-r','LineWidth',1.0);

    ND_display_data.display2Dmap(centroid_struct.Thcentroids.*mask,'figNum',figNum+24,'Xval',dat.xsuperGrid_s(1,:),'Yval',dat.ysuperGrid_s(:,1),'window',window_for_maps,'figTitle','Theta Centroids ','font',30,'size_figure',[90 151 1203 510]);caxis([84.28 84.5])
    figure(figNum+24);  hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),mask(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),'-r','LineWidth',1.0);

    ND_display_data.display2Dmap(struct_centroidShift.strain.*mask,'figNum',figNum+25,'Xval',dat.xsuperGrid_s(1,:),'Yval',dat.ysuperGrid_s(:,1),'window',window_for_maps,'figTitle','Strain','font',30,'size_figure',[90 151 1203 510]);%caxis([220 250]);
    %figure(figNum+25);  hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),mask(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),'-r','LineWidth',1.0);
    %figure(figNum+25); hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),struct_centroidShift.strain(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),distr_struct.strain,'LineWidth',3.0,'Color','k');
   
    
    ND_display_data.display2Dmap(struct_centroidShift.tilt_x.*mask,'figNum',figNum+26,'Xval',dat.xsuperGrid_s(1,:),'Yval',dat.ysuperGrid_s(:,1),'window',window_for_maps,'figTitle','Tilt around x ','font',30,'size_figure',[90 151 1203 510]);%caxis([100 135]);
    figure(figNum+26);  hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),mask(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),'-r','LineWidth',1.0);
   
    ND_display_data.display2Dmap(struct_centroidShift.tilt_y.*mask,'figNum',figNum+27,'Xval',dat.xsuperGrid_s(1,:),'Yval',dat.ysuperGrid_s(:,1),'window',window_for_maps,'figTitle','Tilt around y ','font',30,'size_figure',[90 151 1203 510]);%caxis([84.28 84.5])
    figure(figNum+27);  hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),mask(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),'-r','LineWidth',1.0);
    ...
    figure(figNum+28);  hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),mask(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),'-r','LineWidth',1.0);
    ...
    figure(figNum+29); hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),struct_centroidShift.dspace(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),fliplr(distr_struct.dspace),'LineWidth',1.0,'Color','k');
...
initial_index = 1;
     ...
     for ii = 1:numel(dat.scan)
        ...
        if ii > initial_index
            xshifts_ini(ii) = 1;%XRF_aligned.scan(ii).xshifts-10;%(ii-initial_index-1)*5;
...
return;
%%%% Analysis of the strain section
...
if flag_strain_analysis == 1
    ...
    contour_values = [1:5:70]*0.01;
    [distr_struct,mask_struct] = ND_analysis.computeStrainContours(dat,struct_centroidShift,contour_values);
    
    
    window_for_maps = [1,34,1,1059]
    
    figNum = 5000;
    
    

    for kk = 1:3:numel(mask_struct)
         
       % ND_display_data.display2Dmap(dat.map2D_SumInt.*mask_struct(kk).mask,'Xval',dat.xsuperGrid_s(:,1),'YVal',dat.ysuperGrid_s(1,:),'window',window_for_maps,'figTitle',[ num2str(contour_values(kk)) '% of Integrated diffraction intensity'],'font',30,'size_figure',[90 151 1203 510],'figNum',figNum+kk);

        %figure(figNum);  hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),mask_struct(kk).mask(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),'-r','LineWidth',3.0);
        
        ND_display_data.display2Dmap(struct_centroidShift.strain.*mask_struct(kk).mask,'figNum',figNum+25+kk,'Xval',dat.xsuperGrid_s(:,1),'YVal',dat.ysuperGrid_s(1,:),'window',window_for_maps,'figTitle',['Strain at ' num2str(contour_values(kk)) '%'],'font',30,'size_figure',[90 151 1203 510]);%caxis([220 250]);
        %figure(figNum+25+kk);  hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),mask(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),'-r','LineWidth',1.0);
        caxis([-.001 .002])
    end
    ...
    for kk = 1:3:numel(mask_struct)
        ...
        ND_display_data.display2Dmap(struct_centroidShift.dspace.*mask_struct(kk).mask,'figNum',figNum+25+kk,'Xval',dat.xsuperGrid_s(:,1),'YVal',dat.ysuperGrid_s(1,:),'window',window_for_maps,'figTitle',['D space at ' num2str(contour_values(kk)) '%'],'font',30,'size_figure',[90 151 1203 510]);%caxis([220 250]);
        %figure(figNum+25+kk);  hold on;contour(dat.xsuperGrid_s(1,window_for_maps(3):window_for_maps(4)),dat.ysuperGrid_s(window_for_maps(1):window_for_maps(2),1),mask(window_for_maps(1):window_for_maps(2),window_for_maps(3):window_for_maps(4)),'-r','LineWidth',1.0);
        caxis([3.73342821813895 3.73342821813905])
    end
    
    
    figure(figNum+25+kk+1);clf;
    %subplot(121);
    plot(contour_values,distr_struct.strain);
    xlabel('% of diffracted intensity map');
    ylabel('strain \Delta d/d');
    ...
    figure(figNum+25+kk+2);clf;
    %subplot(122);
    plot(contour_values,distr_struct.dspace);
    xlabel('% of diffracted intensity map');
    ylabel('D [Angstroms]');
    set(gca,'FontSize',30)
