...
final_index = index+1;
     ...
     for ii = 1:numel(dat.scan)
        ...
        if ii > initial_index %&& ii < final_index
            xshifts_ini(ii) = 3;%XRF_aligned.scan(ii).xshifts-10;%(ii-initial_index-1)*5;
    ...
    ND_display_data.displayAlignedXRFmaps(XRF_aligned_toshow,'figNum',index+1);
...
%{
pixel = [18,49]; % rows and columns, Y and X
